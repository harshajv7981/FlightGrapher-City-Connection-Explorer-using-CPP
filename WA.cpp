...
void routeSearch_2(Graph graph, int city_A, int city_B, int city_C, int city_D) {
    ...
    // Find route1: from city_A to city_B
    Dijkstra(graph, city_A, d, p);
    if (d[city_B] <= INT_MAX) {
        int cur_city = city_B;
        while (cur_city != -1) {
        ...
        // No route1 exists
        cout << "No route from city A to city B." << endl;
        return;
    }

    // Find route2: from city_B to city_C
    Dijkstra(graph, city_B, d, p);
    if (d[city_C] <= INT_MAX) {
        int cur_city = city_C;
        while (cur_city != city_B) {
            ans.push_back(cur_city);
            cur_city = p[cur_city];
        }
    } else {
        // No route2 exists
        cout << "No route from city B to city C." << endl;
        return;
    }

    // Find route3: from city_C to city_D
    Dijkstra(graph, city_C, d, p);
    if (d[city_D] <= INT_MAX) {
        int cur_city = city_D;
        while (cur_city != city_C) {
            ans.push_back(cur_city);
            cur_city = p[cur_city];
        }
    } else {
        // No route3 exists
        cout << "No route from city C to city D." << endl;
        return;
    }

    // Print the final route
    cout << "The route with the smallest number of connections from city A to D through cities B and C:" << endl;
    ...
    cout << endl;
    cout << "Total connections: " << ans.size() - 1 << endl;
}
